...
export const SETTING_DEFINITIONS: SettingDefinition[] = [
	...
	{
		key: "worker_pi_auth_mount_source",
		type: "string",
		description: "Docker volume or bind path holding the shared pi auth.json (OpenAI Codex OAuth credentials) mounted read-only into worker containers so they can read and refresh OAuth tokens.",
		default: "yolomatic_pi",
		requiresRestart: true,
		sensitive: false,
		envVar: "YOLO_WORKER_PI_AUTH_MOUNT_SOURCE",
		category: "agent-behavior",
	},
	{
		key: "worker_pi_auth_dir",
		type: "string",
		description: "In-container path where the shared pi auth volume is mounted read-only for workers; forwarded as PI_CODING_AGENT_DIR so AuthStorage reads auth.json from there.",
		default: "/home/yolomatic/.pi/agent",
		requiresRestart: true,
		sensitive: false,
		envVar: "YOLO_WORKER_PI_AUTH_DIR",
		category: "agent-behavior",
	},
	{
	...
	},
	{
		...
		type: "string",
		description: "LLM provider used by worker containers. Supported providers: ollama, openai (OpenAI platform API key), openai-codex (ChatGPT Plus/Pro OAuth).",
		default: "ollama",
		requiresRestart: false,
		sensitive: false,
