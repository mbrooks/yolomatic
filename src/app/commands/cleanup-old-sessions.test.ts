...
describe("CleanupOldSessions", () => {
	it("deletes stale terminal sessions and removes their worktrees", async () => {
		...
		expect(workspaces.removeWorktree).toHaveBeenCalledWith("mbrooks", "tars", 341);
		expect(workspaces.removeWorktree).not.toHaveBeenCalledWith("mbrooks", "tars", 342);
		expect(sessions.delete).toHaveBeenCalledWith("mbrooks", "tars", 341);
	...
	it("increments failed and does not delete the session when removeWorktree throws", async () => {
		...
		expect(writeSpy).toHaveBeenCalledWith(expect.stringContaining("failed to delete mbrooks/tars#341"));
