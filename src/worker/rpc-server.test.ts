...
describe("WorkerRpcServer", () => {
	it("accepts a pending worker session and buffers early messages", async () => {
		...
		const pending = rpcServer.createPendingConnection("mbrooks/yolomatic#77");
		const { client, socket } = createUpgradeSocket();

		server.emit(
			"upgrade",
			{ url: `${WORKER_RPC_PATH}?sessionKey=mbrooks%2Fyolomatic%2377&token=${pending.token}` },
			socket,
			Buffer.alloc(0),
		);
		...
		await sendWorkerWebSocketMessage(
			client as never,
			createWorkerMessage("hello", "mbrooks/yolomatic#77", "msg-1", { workerVersion: "test", pid: 1 }),
		);
		...
		try {
			...
			await connection.send(createWorkerMessage("ack", "mbrooks/yolomatic#77", "ack-1", { ackMessageId: "msg-1" }));
	...
	it("rejects unauthorized upgrade attempts", async () => {
		const { server, rpcServer, close } = createHarness();
		const pending = rpcServer.createPendingConnection("mbrooks/yolomatic#78");
		...
		try {
			server.emit(
				"upgrade",
				{ url: `${WORKER_RPC_PATH}?sessionKey=mbrooks%2Fyolomatic%2378&token=wrong` },
				socket,
				Buffer.alloc(0),
			);
	...
	it("rejects pending connections when disposed or closed", async () => {
		...
		try {
			const disposed = rpcServer.createPendingConnection("mbrooks/yolomatic#79");
			disposed.dispose(new Error("disposed"));
			await expect(disposed.waitForConnection()).rejects.toThrow("disposed");

			const closed = rpcServer.createPendingConnection("mbrooks/yolomatic#80");
			await rpcServer.close();
			await expect(closed.waitForConnection()).rejects.toThrow("closed before mbrooks/yolomatic#80 connected");
		} finally {
			await close();
		}
	});
	...
	it("buffers websocket errors and reports closed connections immediately", async () => {
		const { server, rpcServer, close } = createHarness();
		const pending = rpcServer.createPendingConnection("mbrooks/yolomatic#81");
		const { client, socket } = createUpgradeSocket();

		server.emit(
			"upgrade",
			{ url: `${WORKER_RPC_PATH}?sessionKey=mbrooks%2Fyolomatic%2381&token=${pending.token}` },
			socket,
			Buffer.alloc(0),
		);
	...
	it("ignores unrelated upgrade requests and closes safely before attach", async () => {
		...
		const { server, rpcServer, close } = createHarness();
		const pending = rpcServer.createPendingConnection("mbrooks/yolomatic#82");
		void pending.waitForConnection().catch(() => undefined);
		const { socket } = createUpgradeSocket();
	...
	it("closes active connections and ignores dispose after a worker is connected", async () => {
		const { server, rpcServer, close } = createHarness();
		const pending = rpcServer.createPendingConnection("mbrooks/yolomatic#83");
		const { client, socket } = createUpgradeSocket();

		server.emit(
			"upgrade",
			{ url: `${WORKER_RPC_PATH}?sessionKey=mbrooks%2Fyolomatic%2383&token=${pending.token}` },
			socket,
			Buffer.alloc(0),
		);
	...
	it("ignores missing upgrade URLs, rejects mismatched sessions, and supports unsubscribing error listeners", async () => {
		...
		const mismatched = rpcServer.createPendingConnection("mbrooks/yolomatic#84");
		void mismatched.waitForConnection().catch(() => undefined);
		const mismatchSocket = createUpgradeSocket();
		server.emit(
			"upgrade",
			{ url: `${WORKER_RPC_PATH}?sessionKey=mbrooks%2Fyolomatic%23999&token=${mismatched.token}` },
		...
		const pending = rpcServer.createPendingConnection("mbrooks/yolomatic#85");
		const { client, socket } = createUpgradeSocket();
		server.emit(
			"upgrade",
			{ url: `${WORKER_RPC_PATH}?sessionKey=mbrooks%2Fyolomatic%2385&token=${pending.token}` },
			socket,
			Buffer.alloc(0),
		);
