...
describe("SelfMonitor", () => {
	it("records tool history and detects fatal errors", async () => {
		const dir = await mkdtemp(path.join(os.tmpdir(), "yolomatic-sm-"));
	...
	it("formats a bug report body", async () => {
		...
		expect(body).toContain("yolomatic");
	...
	it("returns hardcoded target repo", () => {
		expect(SelfMonitor.getTargetRepo()).toEqual({ owner: "mbrooks", repo: "yolomatic" });
	});
