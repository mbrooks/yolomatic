...
describe("repo-settings api", () => {
	...
	it("fetches repo settings", async () => {
		...
		await expect(fetchRepoSettings("mbrooks", "tars")).resolves.toEqual({ settings: [] });
		expect(fetchSpy).toHaveBeenCalledWith("/api/repos/mbrooks/tars/settings");
	});
	...
	it("updates repo settings", async () => {
		...
		await expect(updateRepoSettings("mbrooks", "tars", { github_event_mode: "polling" })).resolves.toEqual({
	...
	it("surfaces API errors", async () => {
		...
		await expect(updateRepoSettings("mbrooks", "tars", { github_event_mode: "bad" })).rejects.toThrow("bad settings");
	});
});
