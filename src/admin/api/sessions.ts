...
export const SESSION_ACTIONS: readonly SessionActionConfig[] = [
	{
		...
		variant: "stop",
		confirmMessage: (s) => `Stop TARS on ${s.owner}/${s.repo}#${s.issueNumber}?`,
		command: () => ({ type: "cancel" }),
	...
	},
	{
		...
		variant: "pause",
		confirmMessage: (s) => `Pause TARS on ${s.owner}/${s.repo}#${s.issueNumber}?`,
		command: () => ({ type: "pause" }),
	...
	},
	{
		...
		variant: "resume",
		confirmMessage: (s) => `Resume TARS on ${s.owner}/${s.repo}#${s.issueNumber}?`,
		command: () => ({ type: "resume" }),
