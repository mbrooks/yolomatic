...
export const SESSION_ACTIONS: readonly SessionActionConfig[] = [
	{
		...
		variant: "stop",
		confirmMessage: (s) => `Stop Yeetomatic on ${s.owner}/${s.repo}#${s.issueNumber}?`,
		command: () => ({ type: "cancel" }),
	...
	},
	{
		...
		variant: "pause",
		confirmMessage: (s) => `Pause Yeetomatic on ${s.owner}/${s.repo}#${s.issueNumber}?`,
		command: () => ({ type: "pause" }),
	...
	},
	{
		...
		variant: "resume",
		confirmMessage: (s) => `Resume Yeetomatic on ${s.owner}/${s.repo}#${s.issueNumber}?`,
		command: () => ({ type: "resume" }),
