...
Yolomatic persists runtime state in SQLite databases opened with `node:sqlite`
...
Backs `SkillsStore`. Stores user-authored Yolomatic skills (`name`, `description`,
...
| `seen_at`    | TEXT | `NOT NULL`         | When Yolomatic first recorded it (ISO timestamp).                     |
...
| `branch`              | string (optional)                     | Branch associated with the session (defaults to `yolomatic/issue-${issueNumber}`). |
