...
implemented by Yolomatic, plus explicitly labeled proposed extensions:

- Yolomatic remains the deterministic control plane.
...
- The worker has no GitHub credentials, no Docker socket, and no direct access to Yolomatic state.
- Yolomatic and the worker communicate through a bidirectional session protocol over a WebSocket connection.
...
- [issue-refinement.md](issue-refinement.md): proposed opt-in workflow for
  ...
  Yolomatic's built-in prompt defaults, reusing the disposable Docker worker
  for investigation and testing, and automatically replacing the issue body
  after an authorized `/yolomatic issue-refinement` command
- [architecture.md](architecture.md): system overview, trust boundaries, and lifecycle
- [protocol-launch.md](protocol-launch.md): how Yolomatic starts a worker session and exposes the worker session URL
...
- Keep orchestration, repo management, push, and PR delivery in Yolomatic.
...
- In Docker Compose deployments, let workers share the Yolomatic container network namespace so they reach Yolomatic and Ollama over `127.0.0.1` instead of a host-published port.
