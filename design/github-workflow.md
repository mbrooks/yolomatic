...
Yolomatic: how repository activity enters the control plane, how an issue is
...
- The issue branch is `yolomatic/issue-{number}` and the worktree is
...
- Yolomatic never merges its own pull requests or closes the source issue.
...
- the event was emitted by the configured Yolomatic GitHub account;
- the issue has `wontfix` or `invalid`;
- an `opened` or `assigned` issue is not assigned to Yolomatic;
...
authored by a non-bot account other than Yolomatic, and the issue is assigned
to Yolomatic. Payloads identified as belonging to a closed issue are rejected.
The issue must be assigned to Yolomatic and the comment must explicitly
trigger feedback by mentioning the configured account or `@yolomatic`, or by
containing the `/yolomatic feedback` comment command. The Yolomatic-visible
label is no longer part of the comment gate (neither required nor sufficient).
A mention still adds the `yolomatic` routing-marker label, but that label no
longer affects gate eligibility. When a qualifying trigger comment is accepted,
Yolomatic gathers the issue's prior non-trigger, non-Yolomatic-authored
...
Yolomatic should offer to expand a newly opened issue into a Proposed Task.
...
implementation, using an exact `/yolomatic issue-refinement` comment. That
...
follows Yolomatic's built-in issue-refinement defaults from the prompt. It may
...
Yolomatic GitHub account, then enters the same session and execution flow.
...
3. It creates the `yolomatic/issue-{number}` worktree. An empty repository is
...
5. The issue is marked `yolomatic-working` and receives the pickup comment.
...
4. Fetches remote branches, temporarily stashes dirty changes, and
   fast-forwards from `origin/yolomatic/issue-{number}` when that branch
   exists.
...
Any preflight failure marks the session failed, adds `yolomatic-failed`, and
...
| --- | --- | --- |
| `working` | `working` | Keep or restore `yolomatic-working`; post progress summary. |
| `waiting-feedback` | `waiting-feedback` | Add `yolomatic-feedback-required`; post the clarification request. |
| `cancelled` | `cancelled` | Add `yolomatic-cancelled`; post cancellation. |
| `failed` | `failed` | Add `yolomatic-failed`; post failure details. |
| `complete` with changes | `complete` | Commit and push, open a draft PR, gate on GitHub mergeability (rework conflicts up to twice), then flip ready, add `yolomatic-pr-created`, and post the PR link. |
| `complete` without changes | `complete` | Post the summary and state that no code changes were necessary. |

Before normal issue transitions, Yolomatic removes
`yolomatic-working`, `yolomatic-feedback-required`,
`yolomatic-pr-created`, and `yolomatic-complete`. The `yolomatic` label is a
...
2. Creates a commit when staged changes exist, using an issue-aware generated
   commit message.
3. Pushes `yolomatic/issue-{number}` when the new commit or an already-local
   commit is ahead of the base.
4. Creates a **draft** PR titled `Yolomatic: {issue title}` against the
...
7. Runs a mergeability gate against `octokit.pulls.get`: polls `mergeable`
   ...
   `yolomatic-pr-created` is applied, and "Ready for
   ...
   failed via `yolomatic-delivery-failed` + `yolomatic-working`, the
   worktree is preserved, and the PR stays a draft.

If GitHub reports that a PR already exists, Yolomatic finds the open PR with
...
Delivery failures are distinct from worker failures. Yolomatic preserves the
worktree, captures Git status/diff diagnostics, marks the session failed, and
adds both `yolomatic-working` and `yolomatic-delivery-failed` so the work is
not mistaken for an agent failure. When self-reporting is enabled it also files
a bug against the configured Yolomatic repository.

## Feedback and PR Iterations

### Issue Feedback

For an idle, eligible issue comment, Yolomatic reuses the existing session and
...
Before execution, Yolomatic requires all of the following:

- the event is not from the Yolomatic account;
- the PR is open and unmerged;
- the branch is either `yolomatic/issue-{number}` with a session explicitly
...
1. Yolomatic atomically registers the issue session and posts the iteration
   number.
...
Yolomatic-owned PR timelines. An active task receives an abort signal; a
working session without an active task is marked cancelled; an idle session
receives a no-op response.

### Unassignment

When an `issues.unassigned` payload shows that Yolomatic is no longer among
...
On startup, Yolomatic resumes sessions with `resumeOnBoot` or `working`
...
- The worker can modify the shared workspace and use the network, but it
  cannot call GitHub as Yolomatic, access the Docker socket, or read control
...
- Delivery errors preserve local changes for recovery and use the separate
  `yolomatic-delivery-failed` label.
...
- Workflow cleanup does not remove `yolomatic-failed`,
  `yolomatic-cancelled`, or `yolomatic-delivery-failed`; those labels can
...
- Yolomatic does not merge PRs or close issues. The PR-creation delivery path
